...
import { Search, Filter, ChevronDown, Eye, Package, Clock, CheckCircle, XCircle, Truck } from "lucide-react";
import { useState, useMemo } from "react";

const orders = [
    { id: "ORD-2026-001", date: "Jan 10, 2026", customer: "Priya Sharma", items: 2, total: 6897, status: "Processing", payment: "Prepaid" },
    { id: "ORD-2026-002", date: "Jan 10, 2026", customer: "Rahul Verma", items: 1, total: 2499, status: "Shipped", payment: "Prepaid" },
    { id: "ORD-2026-003", date: "Jan 9, 2026", customer: "Anita Desai", items: 3, total: 8750, status: "Delivered", payment: "COD" },
    { id: "ORD-2026-004", date: "Jan 9, 2026", customer: "Vikram Singh", items: 1, total: 1599, status: "Pending", payment: "Prepaid" },
    { id: "ORD-2026-005", date: "Jan 8, 2026", customer: "Meera Patel", items: 2, total: 4599, status: "Delivered", payment: "Prepaid" },
    { id: "ORD-2026-006", date: "Jan 8, 2026", customer: "Arjun Kumar", items: 1, total: 2199, status: "Cancelled", payment: "COD" },
    { id: "ORD-2026-007", date: "Jan 7, 2026", customer: "Sneha Reddy", items: 4, total: 12500, status: "Delivered", payment: "Prepaid" },
    { id: "ORD-2026-008", date: "Jan 7, 2026", customer: "Karan Malhotra", items: 1, total: 3299, status: "RTO", payment: "COD" },
];
...
const sortOptions = [
    ...
    { label: "Amount: High to Low", value: "total-desc" },
    { label: "Amount: Low to High", value: "total-asc" },
];
...
export default function OrdersPage() {
    ...
    const filteredOrders = useMemo(() => {
        ...
        result.sort((a, b) => {
            let comparison = 0;
            if (field === "date") comparison = new Date(a.date).getTime() - new Date(b.date).getTime();
            else if (field === "total") comparison = a.total - b.total;
            ...
            {showFilters && (
                <div className="bg-white rounded-xl border border-border p-4 animate-fade-in">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button onClick={() => { setStatusFilter("All"); setPaymentFilter("All"); }} className="text-xs text-primary">Clear all</button>
                    </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr className="bg-surface-hover">
                            ...
                            <th className="text-left text-xs font-semibold text-muted uppercase px-6 py-3">Items</th>
                            <th className="text-left text-xs font-semibold text-muted uppercase px-6 py-3">Total</th>
                            <th className="text-left text-xs font-semibold text-muted uppercase px-6 py-3">Payment</th>
                        ...
                        {filteredOrders.map((order) => {
                            ...
                            return (
                                <tr key={order.id} className="hover:bg-surface-hover/50">
                                    ...
                                    <td className="px-6 py-4 text-sm text-foreground">{order.payment}</td>
                                    <td className="px-6 py-4">
